        ...
        for (int i = 0; i < outCount; i++) {
            ...
            if (range.location != NSNotFound) {
                ...
                if (![type hasPrefix:@"NS"]) {
                    ...
                    // 获取到每个模型的类型
                    id class ;
                    if ([self respondsToSelector:@selector(nameOfObjectClassInArray)]) {
                        
                        NSString *classStr = [self nameOfObjectClassInArray];
                    ...
                    for (int i = 0; i < array.count; i++) {
                        [mArray addObject:[class objectWithDic:value[i]]];
                    }
